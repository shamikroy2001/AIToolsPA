...
2. Railway **API**: Root Directory `backend`, Dockerfile `backend/Dockerfile` (health `/health`). Start is `alembic upgrade head && uvicorn`.
...
6. Stripe **test** webhook → `https://<api>/api/webhooks/stripe`. Credits only after the webhook.
...
Railway root directory: `backend/`. Vercel root: `frontend/`.
